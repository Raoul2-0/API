class DecodeConfigurationService < TableService
  ...
  def make_block(element)
    ...
    
    {
      ...
      decodes: element.school_decodes(@current_school.id),
  ...
  def decodes_number(element)
    my_cel = cel([{ class: "decodes_number" }])
    size = micro_cel([{ class: "size"}], element.school_decodes(@current_school.id).length)

    my_cel[:list] = [size].compact
